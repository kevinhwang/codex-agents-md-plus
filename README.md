...
| **Walk anchor**             | mutable `originalCwd` per-agent (resets on `/cd`, `EnterWorktree`) | session-start cwd                                               | per-event: session uses rollout-transcript cwd; subagent uses its own cwd |
...
The plugin emits a single block wrapped in `<agents_context>…</agents_context>` as `additionalContext`. Codex inserts it into the conversation transcript as a developer-role message. Inside, each file's body is wrapped in its own per-file `<file:… path="…">` tag. Subsections:
...
- **Referenced document index** — `@`-expanded documents, with the reference chain as a `referenced-from` attribute.
