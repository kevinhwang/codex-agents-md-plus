    ...
    overlay = Overlay(
        ...
        references=(
            ReferenceDoc(
                ...
                text="ref body",
                truncated=False,
                chain=(tmp_path / "AGENTS.local.md",),
            ),
        ),
...
def test_outer_container_wraps_body(tmp_path: Path) -> None:
    ...
    body = render(overlay)
    assert body.startswith("<agents_context>")
    assert body.rstrip().endswith("</agents_context>")
...
def test_reference_doc_uses_referenced_from_attribute(tmp_path: Path) -> None:
    overlay = Overlay(
        ...
        references=(
            ReferenceDoc(
                ...
                text="ref body",
                truncated=False,
                chain=(tmp_path / "AGENTS.md", tmp_path / "intermediate.md"),
            ),
        ),
        skipped=(),
    )
    ...
    assert str(tmp_path / "AGENTS.md") in body
    assert str(tmp_path / "intermediate.md") in body
