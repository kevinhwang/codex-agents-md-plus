...
# per-file `<file:HASH path="...">` wrappers, the outer `<agents_context>`
...
def expand(
    ...
    for seed in seeds:
        _enqueue_from(pending, seed.path, seed.text, chain=(seed.path,), depth=1)
    ...
    while pending:
        ...
        seen.add(target)
        references_out.append(ReferenceDoc(path=target, text=text, truncated=truncated, chain=ref.chain))

        _enqueue_from(pending, target, text, chain=(*ref.chain, target), depth=ref.depth + 1)
